...
@click.group()
@click.version_option("0.3.1", prog_name="plaud")
@click.option(
    "--config", "config_path",
...
@click.option("--hydrate/--no-hydrate", default=True, show_default=True)
def export(file_id: str, token: str | None, fmt: str, output: str | None, hydrate: bool) -> None:
    ...
    if fmt == "json":
        rendered = json.dumps(norm, indent=2, ensure_ascii=False)
    elif fmt == "txt":
        rendered = _render_txt(norm)
    else:
        rendered = _render_markdown(norm)
...
def _render_markdown(norm: dict[str, Any]) -> str:
    lines: list[str] = []
...
def _render_txt(norm: dict[str, Any]) -> str:
    title = norm["title"] or norm["file_id"]
    lines: list[str] = [
        title,
        "=" * len(title),
